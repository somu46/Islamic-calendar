...
const FullYearHijriCalendar = ({ year }) => {
  ...
  const fetchFullYearData = useCallback(async () => {
    ...
    try {
      const promises = Array.from({ length: 12 }, (_, index) => {
        ...
        return axios.get(`https://api.aladhan.com/v1/gToHCalendar/${formattedMonth}/${year}`);
      });
  ...
  const renderMonth = (monthData, monthIndex) => {
    const daysInMonth = Array.from({ length: new Date(year, monthIndex + 1, 0).getDate() }, (_, i) => i + 1);
    const firstDayOfMonth = new Date(year, monthIndex, 1).getDay();
    
    const days = Array(firstDayOfMonth).fill(null).concat(daysInMonth);

    return (
      <div key={monthIndex} className="border rounded-lg p-4 shadow-md bg-white">
        <h3 className="text-lg font-bold text-center mb-2">{monthNames[monthIndex]}</h3>
          ...
          {days.map((day, index) => {
            ...
            return (
              <div key={index} className="flex justify-center">
                <div
                  className={`flex flex-col items-center justify-center w-10 h-10 rounded-md cursor-pointer 
                    ${!day ? 'bg-transparent' : ''} 
                    ${holiday ? 'bg-red-200' : ''} 
  ...
  return (
    <div className="container mx-auto p-4 bg-gradient-to-b from-blue-50 to-blue-100 min-h-screen">
      <h2 className="text-2xl font-bold text-center text-green-700 mb-4">Hijri Calendar - {year}</h2>
      {loading ? (
        <div className="text-center">Loading...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {calendarData.map((monthData, monthIndex) => renderMonth(monthData, monthIndex))}
        </div>
      )}
    </div>
  );
